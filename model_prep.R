model_prep <- function(words_dfm){
  combined_1grams <- dfm(tokens_ngrams(combined_tok, 1))
  p_1s <- good_turing(combined_1grams)
  p_1s <- mutate(p_1s, ngram = 1)
  
  combined_2grams <- dfm(tokens_ngrams(combined_tok, 2))
  p_2s <- good_turing(combined_2grams)
  p_2s <- mutate(p_2s, ngram = 2)
  
  combined_3grams <- dfm(tokens_ngrams(combined_tok, 3))
